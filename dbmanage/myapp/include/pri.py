...
from dbmanage.myapp.models import Db_name,Db_group,Db_account,Db_instance,Oper_log,Login_log,MySQL_monitor
...
def get_full():
    ...
    userlist = UserInfo.objects.all().order_by('username')
...
def set_dbgroup(groupname,dbnamesetlist,usersetlist):
    ...
    new_user = UserInfo.objects.filter(username__in=usersetlist)
    ...
    #del user handle
    for i in UserInfo.objects.filter(username__in=del_user):
        try:
    ...
    #add user handle
    for i in UserInfo.objects.filter(username__in=add_user):
        try:
            gp.account.add(i)
            for x in new_db:
                try:
    ...
    #exists user handle
    for i in UserInfo.objects.filter(username__in=inter_user):
        existdbli = get_exist_db(groupname, i, old_dbli)
...
def create_dbgroup(groupname,dbnamesetlist,usersetlist):
    ...
    myuser = UserInfo.objects.filter(username__in=usersetlist)
...
def get_full_per():
    ...
    c = UserInfo.objects.all().order_by('username')
    return a,b,c
...
def create_ugroup(groupname,persetlist,usersetlist):
    ...
    userli = UserInfo.objects.filter(username__in=usersetlist)
    for i in perli:
        try:
...
def get_fulldbname():
    ...
    c = UserInfo.objects.exclude(username=func.public_user).order_by('username')
    return a,b,c
...
def create_dbtag(dbtagname,newdbname,inssetlist,usersetlist):
    if len(dbtagname)>0 and len(newdbname)>0:
        ...
        useli = UserInfo.objects.filter(username__in=usersetlist)
    for i in accli:
        try:
...
def set_acc(old_account,tags,user,passwd,dbtagli,acclist,role):
    ...
    for i in Db_name.objects.filter(dbtag__in=dbtagli):
        try:
            ...
            pass
    for i in  UserInfo.objects.filter(username__in=acclist):
        try:
...
def createdb_fast(ins_set, newinsip, newinsport, newdbtag, newdbname, newname_all, newpass_all, newname_admin,newpass_admin,new_instype):
    ...
    try:
        user = UserInfo.objects.get(username=func.public_user)
...
def check_pubuser():
    ...
    try :
        tmp = UserInfo.objects.get(username=pubuser)
    except Exception,e:
        passwd = str(uuid.uuid1()).split('-')[0]
        user = UserInfo.objects.create_user(username=pubuser,email='public@test.com',password=passwd)
        user.save()
