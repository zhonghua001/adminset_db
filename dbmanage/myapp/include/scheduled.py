...
from dbmanage.myapp.include import inception as incept
...
from dbmanage.myapp.models import User_profile
...
def task_sche_run():
    try:
        ...
        task = Task.objects.filter(status='appointed').filter(sche_time__lte=timezone.now())
        if len(task)>0:
            for mytask in task:
                print "mytask_id"
                print mytask.id
