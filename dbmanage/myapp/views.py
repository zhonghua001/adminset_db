...
from form import AddForm, LoginForm, Logquery, Uploadform, Taskquery, Taskscheduler
...
from dbmanage.myapp.models import Db_group, Db_name, Db_account, Db_instance, Task
from dbmanage.myapp.tasks import task_run, sendmail_task, parse_binlog, parse_binlogfirst
...
def index(request):
    ...
    # return render(request, '',{'inc_data':json.dumps(inc_data),'inc_col':json.dumps(inc_col),'bingtu':json.dumps(bingtu),'data':json.dumps(data),'col':json.dumps(col),'taskdata':json.dumps(taskdata),'taskcol':json.dumps(taskcol),'temp_name':temp_name})
...
def log_query(request):
    ...
    if request.method == 'POST' :
        ...
        if form.is_valid():
            ...
            return render(request,'log_query.html',{'form': form,'objlist':obj_list,'optypelist':optype_list,'datalist':data,'choosed_host':hosttag,'temp_name':temp_name})
        else:
            print "not valid"
            return render(request,'log_query.html',{'form': form,'objlist':obj_list,'optypelist':optype_list,'temp_name':temp_name})
    else:
        form = Logquery()
        return render(request, 'log_query.html', {'form': form,'objlist':obj_list,'optypelist':optype_list,'temp_name':temp_name})
...
def task_manager(request):
    ...
    if request.method == 'POST' :
        ...
        if request.POST.has_key('commit'):
            # data = incept.get_task_list(hosttag,request,endtime)
            return render(request,'task_manager.html',{'form':form,'form2':form2,'objlist':obj_list,'datalist':data,'choosed_host':hosttag,'temp_name':temp_name})
            ...
            return render(request,'task_manager.html',{'form':form,'form2':form2,'objlist':obj_list,'datalist':data,'choosed_host':hosttag,'result':results,'col':col,'temp_name':temp_name})
            ...
            return render(request,'task_manager.html',{'form':form,'form2':form2,'objlist':obj_list,'datalist':data,'choosed_host':hosttag,'result_status':results,'cols':cols,'temp_name':temp_name})
            ...
            # print nllflag
            return render(request,'task_manager.html',{'form':form,'form2':form2,'objlist':obj_list,'datalist':data,'choosed_host':hosttag,'nllflag':nllflag,'temp_name':temp_name})
        ...
        data = incept.get_task_list('all',request,timezone.now())
        form = Taskquery()
        form2 = Taskscheduler()
        return render(request, 'task_manager.html', {'form':form,'form2':form2,'objlist':obj_list,'datalist':data,'temp_name':temp_name})
